...
export class AjouterEmploisComponent implements OnInit {
  ...
  onSubmit(){
    ...
    if(this.id === undefined){
      this.emploiService.addEmploi(this.emploi).subscribe((data)=>this.router.navigateByUrl('/BackOffice/emplois'));  
    }else{
      this.emploiService.updateEmploi(this.id,this.emploi).subscribe((data)=>this.router.navigateByUrl('/BackOffice/emplois'));  
